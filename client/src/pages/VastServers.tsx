...
export default function VastServers() {
  ...
  const { data: launchedServers, isLoading: isLoadingLaunched, refetch: refetchServers } = useQuery<VastServer[]>({
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => queryClient.invalidateQueries({ queryKey: ['/api/vast-servers/available'] })}
                  disabled={isLoadingAvailable}
                  className="flex items-center gap-2"
                >
                  <RefreshCw className={`h-4 w-4 ${isLoadingAvailable ? 'animate-spin' : ''}`} />
                  Refresh
                </Button>
              </div>
            </CardHeader>
