...
- June 19, 2025. Implemented automatic server synchronization with Vast.ai API - servers now automatically sync status, connection details, and metadata from live Vast.ai instances with database updates for accurate server tracking. System automatically imports existing Vast.ai instances when API key is configured
```
